...
public class CriteriaFinder<T extends IdentityOwner> {
	...
	protected List<T> setupAndExecuteQuery(List<QueryParameter> parameters) {
		...
		ParameterExpression exp = criteriaBuilder.parameter(p.type);
		query.where(criteriaBuilder.like(root.get(p.name), exp));
		for(int i = 1; i < parameters.size(); i++) {
			p = parameters.get(i);
			exp = criteriaBuilder.parameter(p.type);
			query.where(criteriaBuilder.and(root.get(p.name), exp));
		}
